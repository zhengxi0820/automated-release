...
def s1() -> str:
    ...
    <p class="para dim">截至发稿，官方还没有确认发布时间和定价，以上信息以硅基流动平台页面为准。</p>
...
def s2() -> str:
    ...
    <p class="para">但也要泼一盆冷水：上下文变长，不等于长文中间的内容都能被准确利用。模型是否存在「中间遗忘」、长输入的响应速度如何，都要等实测数据。</p>
...
def s3() -> str:
    ...
    <p class="para">如果 V4 Pro 的 1M 上下文真的稳定，这类活儿就是<b>整份文件直接扔进去</b>，让它自己读完整本再回答。省掉的不只是时间，还有分段导致的信息丢失。</p>
    <p class="para dim">说明：我还没有对 V4 Pro 做过实测，以上是基于参数的推断，周末跑完测试会在这里更新结论。</p>""", 4)


def s4() -> str:
    return section("四", "先小项目试水，别急迁移", """
    <p class="para">给普通人的建议很简单：想尝鲜，去硅基流动把玩一下没问题；但<b>核心工作流别急着迁移</b>。新模型刚上线，稳定性、并发限制、实际价格都是未知数。</p>
    <p class="para">稳妥的路径是：先拿一个<b>不重要的 小项目</b>完整跑一遍，对比旧模型的输出质量和成本，跑通了、满意了，再考虑把主力任务切过来。</p>
